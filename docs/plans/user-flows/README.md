...
> Status: v1.1, created for `br-user-flow-database-1vr` on 2026-06-29 and updated
> after the late-join/map-procgen planning correction on 2026-06-29.
...
2. **Late joins are a product invariant.** Players should be able to join the room in every topology,
   ruleset, and live phase. Fairness is handled by spawn/scoring/eligibility policy, not by blocking
   the join flow.
3. **Server finished state is probably unreachable.** The host emits `end_game`, but no server
   handler was found; late joins cannot be routed cleanly into results/rematch while server state
   remains `racing`.
4. **Host-loss behavior contradicts the Remote/rejoin plan.** The plan wants a grace/reclaim window;
   current server behavior deletes the room immediately on host disconnect.
5. **Local vs Remote and Race vs Derby need separate axes.** Without this, join routing and tests
   become ambiguous.
6. **Mixed topology needs a seat model.** A laptop viewer plus phone controller must bind to one
   seat; the current server model is one socket equals one player.
7. **Derby needs stall and late-entrant policy.** Rounds end only when survivors are `<= 1`; the
   accepted party default queues combat late joiners for the next round, while optional/full-chaos
   active late-entrant rules are now planned behind explicit spawn protection, labels, scoring, and
   playtest gates.
8. **Phone/controller terminal states are incomplete.** Host shows results and lobby transitions;
   controllers do not clearly handle `game_end` or `returned_to_lobby`.
9. **Security/abuse are user-flow problems, not only backend chores.** Remote/mixed play needs seat
   ownership, input spoof rejection, name sanitization, rate limits, and moderator controls.
10. **Map choice and procgen need choose-time structure.** Known and random maps should both resolve
   to a seeded, validated `MapInstance` before Race or Derby start; see
   `docs/plans/map-authoring-and-procedural-generation-2026-06-29.md`.
...
| `UF-MAP-CHOOSE-KNOWN-RANDOM-SEEDED` | Host chooses known, random, or seeded map | any | any | recommended |
...
| `UF-JOIN-LATE-RACE-FINISH-GRACE` | Late race join during finish grace/results | any | race | recommended |
| `UF-LATE-ENTRANT-ACTIVE-OPTION` | Optional active late entrants for full-chaos variants | any | race/derby | planned |
...
| `UF-JOIN-LATE-DERBY-NEXT-ROUND` | Late derby join waits for next round | any | derby | recommended |
...
| `UF-GAP-003` | critical | Host emits `end_game`; server has no matching handler. | Results/rematch late-join routing tests. |
...
| `UF-GAP-011` | high | Derby late join currently enters active round, but accepted default queues combat joiners for next round. | Next-round late-join policy. |
...
| `UF-GAP-020` | high | Finished-late-join E2E is explicitly skipped. | Results/rematch late admission. |
...
| `UF-GAP-024` | high | Late join after first finisher must be admitted without extending the timer or changing locked placements. | Finish-grace late-entrant rules. |
| `UF-GAP-025` | high | Known/random map choice is not a seeded choose-time `MapInstance`. | Map catalog and seed contract. |
| `UF-GAP-026` | high | Procedural generation only covers flat race splines; Derby terrain/jumps are under-specified. | Procgen recipe pipeline. |
...
| `T-ROOM-FINISHED-ADMITS-LATE-JOIN` | Server room enters results/rematch state; late join lands in postgame/rematch, not an error. |
...
| `T-DERBY-LATE-JOIN-NEXT-ROUND` | Mid-round derby join is admitted, waits/spectates, and enters active play next round. |
...
| `T-MAP-CATALOG-FILTERS-RULESET` | Race/Derby map selector prevents hidden cross-ruleset coercion. |
| `T-RANDOM-MAP-REPRODUCES-SEED` | Same recipe/seed/version produces the same map instance. |
| `T-MAP-VALIDATES-LATE-SPAWNS` | Known and random maps validate live late-join spawn capacity. |

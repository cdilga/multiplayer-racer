/**
 * Video capture script for 32-car demo
 * Creates a spectacular demo video showing 32 cars racing simultaneously
 * Run with: npx tsx scripts/capture-32-cars.ts
 *
 * Prerequisites:
...
async function setupLocalRoutes(context: BrowserContext): Promise<void> {
    // Note: Removed CDN interception as it was causing issues
    // Let the browser fetch dependencies normally from CDN
    // This requires internet connection but is more reliable
}
...
async function capture32CarsVideo() {
    if (!fs.existsSync(OUTPUT_DIR)) {
        fs.mkdirSync(OUTPUT_DIR, { recursive: true });
    }

    console.log('🎬 Starting 32-car demo capture...\n');
    console.log(`   Creating ${NUM_CARS} player instances...`);
    ...
    const videoPath = path.join(OUTPUT_DIR, '32-cars-demo.webm');
    ...
    try {
        ...
        for (let batch = 0; batch < Math.ceil(NUM_CARS / BATCH_SIZE); batch++) {
            ...
            console.log(`   Batch ${batch + 1}/${Math.ceil(NUM_CARS / BATCH_SIZE)}: Creating players ${batchStart + 1}-${batchEnd}...`);
        ...
        // Wait a moment to see all players in lobby
        await hostPage.waitForTimeout(2000);
        ...
        console.log('📹 Recording 32 cars in action...');

        // Let the cars just sit there for a moment so we can see all 32
        await hostPage.waitForTimeout(3000);

        // Simulate some movement with keyboard controls for variety
        for (let i = 0; i < 4; i++) {
        ...
        const videoFile = files.find(f => f.endsWith('.webm') && f !== '32-cars-demo.webm' && f !== 'gameplay-demo.webm');
        ...
        if (videoFile) {
            ...
            console.log('\n🔄 Converting to GIF...');
            const gifPath = path.join(OUTPUT_DIR, '32-cars-demo.gif');

            // ffmpeg command for high-quality GIF optimized for 32 cars
            // Use 10fps and 800px width for reasonable file size
            ...
            try {
                ...
                if (gifSize > 10) {
                    ...
                    const compressedGifPath = path.join(OUTPUT_DIR, '32-cars-demo-compressed.gif');
                    const compressCmd = `ffmpeg -y -i "${videoPath}" -vf "fps=8,scale=640:-1:flags=lanczos,split[s0][s1];[s0]palettegen=max_colors=128[p];[s1][p]paletteuse=dither=bayer" -t 12 "${compressedGifPath}"`;
                    ...
                    console.log(`\n💡 Tip: Use the compressed version if needed for README`);
                }
            } catch (ffmpegError) {
        ...
        console.log('\n🎉 32-car demo capture complete!');
        console.log(`📊 Stats:`);
        console.log(`   - Cars: ${NUM_CARS}`);
        console.log(`   - Players joined: ${playerPages.length}`);
        console.log(`   - Video: ${videoPath}`);
        console.log(`   - GIF: ${path.join(OUTPUT_DIR, '32-cars-demo.gif')}`);
